...
class CSA_Layer(nn.Module):
    def __init__(self, channels, activate_function):
        ...
        self.q_conv = nn.Conv1d(3, channels, 1, bias=False)
        self.k_conv = nn.Conv1d(3, channels, 1, bias=False)
    ...
    def forward(self, query, cluster_xyz):       #b,n,3   b,c,n

        cluster_xyz = cluster_xyz.transpose(2, 1).contiguous()
        x_q = self.q_conv(cluster_xyz).permute(0, 2, 1)[:, :, :, None]  # b, n, c,1
        x_k = self.k_conv(cluster_xyz).permute(0, 2, 1)[:, :, None, :]  # b, n, 1,c
        x_v = self.v_conv(query)
...
class GroupFreeDetector(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
                 ...
                 size_cls_agnostic=False, activate_function=None):
        super().__init__()
        ...
        self.csa_feature = CSA_Layer(288, activate_function)
    ...
    def forward(self, inputs):
        ...
        cluster_feature = self.csa_feature(cluster_feature, cluster_xyz)
        # Proposal
        ...
        #query = self.csa_feature(query, cluster_xyz)
